...
FastRcnn_Openvino_Deploy::FastRcnn_Openvino_Deploy(modelConfInfo_ info)
{
...
FastRcnn_Openvino_Deploy::~FastRcnn_Openvino_Deploy()
{
...
void FastRcnn_Openvino_Deploy::get_model_info()
{
...
cv::Mat FastRcnn_Openvino_Deploy::pre_image_process(cv::Mat &image)
{
...
void FastRcnn_Openvino_Deploy::run_model(cv::Mat &input_image)
{
    size_t batch = 1;
    size_t channal = 3;
...
void FastRcnn_Openvino_Deploy::post_image_process(cv::Mat &inputimage)
{
   ...
   if(model == "FasterRcnn")
   {
       labels = (int64*)output_tensor_1.data();
       scores = (float*)output_tensor_2.data();
   }else if(model == "RetinaNet")
   {
       labels = (int64*)output_tensor_2.data();
       scores = (float*)output_tensor_1.data();
   }
...
void FastRcnn_Openvino_Deploy::process()
{
...
// show
void FastRcnn_Openvino_Deploy::set_Show_image(Show *imageShower)
{
    image_show = imageShower;
}

void FastRcnn_Openvino_Deploy::modelRunner()
{
    this->process();
}
