...
keyPointRcnn::keyPointRcnn(std::string modelPath, std::string imagePath, std::string label_text, std::string modelType)
{
...
void keyPointRcnn::get_model_info()
{
...
cv::Mat keyPointRcnn::pre_image_process(cv::Mat &image)
{
...
void keyPointRcnn::run_model(cv::Mat &input_image)
{
    ...
    const std::array<const char*, 5> outNames = { output_node_names[0].c_str(), output_node_names[1].c_str(), output_node_names[2].c_str(),
                                                output_node_names[3].c_str(), output_node_names[4].c_str()};
...
void keyPointRcnn::post_image_process(std::vector<Ort::Value> &outputs, cv::Mat &inputimage)
{
...
// show
void keyPointRcnn::set_Show_image(Show *imageShower)
{
    image_show = imageShower;
}

void keyPointRcnn::modelRunner()
{
    this->process();
}
